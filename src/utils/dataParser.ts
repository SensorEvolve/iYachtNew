...
import { YachtLocation, locationService } from "../services/YachtLocationService";
...
const validateAndFormatDate = (dateString: string): string => {
  try {
    ...
    console.warn(`${LOG_PREFIX} Invalid date format: ${dateString}, using current date`);
    return new Date().toISOString();
  } catch (error) {
    console.warn(`${LOG_PREFIX} Error parsing date: ${dateString}, using current date`, error);
    return new Date().toISOString();
  }
};
...
const parseLocation = (
  ...
  try {
    ...
    if (
      ...
      const timestamp = validateAndFormatDate(entryDate);

      return {
        lat,
        lon,
        speed: Math.max(0, speed), // Ensure non-negative speed
        course: course % 360, // Normalize course to 0-359
        status: 5, // Default to Moored for manual positions
        timestamp,
      };
    } else {
      console.warn(
        `${LOG_PREFIX} Invalid coordinates: lat=${lat}, lon=${lon}`
      );
    }
  } catch (error) {
    console.warn(
...
export const loadYachtData = async (): Promise<Yacht[]> => {
  try {
    ...
    const yachts = lines.map((line, index) => {
      ...
      if (mmsi && !processedMMSIs.has(mmsi)) {
        processedMMSIs.add(mmsi);

        const locationData = parseLocation(locationString, entryDate);
        if (locationData) {
          console.log(
            `${LOG_PREFIX} Found location for MMSI: ${mmsi} | Date: ${locationData.timestamp} | Status: ${locationData.status}`,
          );
